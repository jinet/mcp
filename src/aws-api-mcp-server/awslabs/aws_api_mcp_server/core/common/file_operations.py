...
S3_DOWNLOAD_OPERATIONS: Set[str] = {
    'cp', 'sync', 'mv'
}
...
def is_binary_output_operation(command_metadata: CommandMetadata) -> bool:
    ...
    return (command_metadata.service_sdk_name, command_metadata.operation_sdk_name) in BINARY_OUTPUT_OPERATIONS
...
def is_s3_download_operation(command_metadata: CommandMetadata) -> bool:
    ...
    return (command_metadata.service_sdk_name == 's3' and 
            command_metadata.operation_sdk_name in S3_DOWNLOAD_OPERATIONS)

def extract_file_paths_from_parameters(command_metadata: CommandMetadata, parameters: Dict[str, Any]) -> List[str]:
    ...
    for param_name, param_value in parameters.items():
        if param_name in file_params:
            if isinstance(param_value, str) and not _is_remote_path(param_value):
                ...
                file_paths.extend([item for item in param_value if isinstance(item, str) and not _is_remote_path(item)])
    ...
    if is_binary_output_operation(command_metadata):
        for param_value in parameters.values():
            if isinstance(param_value, str) and not _is_remote_path(param_value) and _could_be_file_path(param_value):
                file_paths.append(param_value)
        
        for param_name in parameters.keys():
            if isinstance(param_name, str) and not _is_remote_path(param_name) and _could_be_file_path(param_name):
